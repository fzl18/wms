<template>
	<view class="container">
		<!-- <u-navbar bgColor="#0051d4" titleStyle="color:#fff;fontSize:16px;fontWeight:bold" height="44" title="物流信息"
			fixed>
		...
		<view>
			<u-sticky bgColor="#fff" class="tab">
			...
			<view class="list" v-if="indexList.length">
				<u-list @scrolltolower="scrolltolower" lowerThreshold="10" :pagingEnabled="true" :scrollTop='top'>
					<u-list-item class="item" v-for="(item, index) in indexList" :key="index">
						<u-cell icon="scan">
							<text slot="title">物流单号: <text class="title-text"> {{item.log_number}}</text></text>
							<!-- <u-icon v-if="!cur" size="20" slot="right-icon" name="more-dot-fill"
								@click="handleOption(item)">
							</u-icon> -->
							<!-- <u-icon name="eye" size="20" slot="right-icon" ></u-icon> -->
						</u-cell>
						<u-cell :border="false" @click="linkto(item)">
							...
							<text slot="value" class="u-cell-text">日期:<text
									class="title-text">{{$u.timeFormat(item.log_time, 'yy-mm-dd')}}</text> </text>
						</u-cell>
						<u-cell :border="false" @click="linkto(item)">
							<text slot="title" class="u-cell-text">收货地址: <text
									class="title-text">{{item.log_shipaddress}}</text></text>
							<text slot="value" class="u-cell-text">重量:<text class="title-text">{{item.log_weight}}</text>
							</text>
						</u-cell>
						<!-- <u-cell :border="false">
	...
	export default {
		...
		methods: {
			...
			linkto(data) {
				...
				this.$store.state.home.itemList = data
				uni.$u.route('/pages/logistics/detail');
			},
			submit() {
