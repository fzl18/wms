<template>
	<view class="container">
		<u-navbar bgColor="#0051d4" titleStyle="color:#fff;fontSize:16px;fontWeight:bold" height="44" title="出库单" fixed>
			...
			<view class="list" v-if="indexList.length">
				<u-list @scrolltolower="scrolltolower" lowerThreshold="10" :pagingEnabled="true" :scrollTop='top'>
					<u-list-item class="item" v-for="(item, index) in indexList" :key="index">
						<u-cell icon="scan">
							<text slot="title">单号: <text class="title-text"> {{item.out_number || item.number}}</text></text>
							<u-icon v-if="!cur" size="26" slot="right-icon" name="share-square"
