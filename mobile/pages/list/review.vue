<template>
	<view class="container">
		<u-navbar bgColor="#0051d4" titleStyle="color:#fff;fontSize:16px;fontWeight:bold" height="44" title="审核"
			fixed>
			...
			<view class="list" v-if="indexList.length">
				<u-list @scrolltolower="scrolltolower" lowerThreshold="10" :pagingEnabled="true" :scrollTop='top'>
					<u-list-item class="item" v-for="(item, index) in indexList" :key="index">
						<u-cell icon="scan">
							<text slot="title" @click="handleEdit(item)">单号: <text class="title-text"> {{item.number}}</text></text>
							<u-switch v-if="!cur" v-model="item.status" :activeValue="1" :inactiveValue="0" @change="(val)=>{handleChange(val,item)}" slot="right-icon"></u-switch>
							<u-switch v-if="cur" v-model="item.rtn_status" :activeValue="1" :inactiveValue="0" @change="(val)=>{handleChange(val,item)}" slot="right-icon"></u-switch>
...
<script>
	...
	export default {
		...
		methods: {
			...
			handleEdit(row){
				row.parmasType = 'edit'
				this.$store.state.home.itemList = row
				uni.$u.route('/pages/list/detail');
			},
			handleOption(data) {
				this.curItem = data
			...
			handleChange(value,row){
				console.log(value,row.rtn_status)
				if(this.cur){
					row.rtn_status = value
				}else{					
					row.status = value
				}
				api.post({
